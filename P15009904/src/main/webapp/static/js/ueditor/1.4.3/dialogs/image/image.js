...
(function () {
    ...
    function initTabs() {
        ...
        for (var i = 0; i < tabs.length; i++) {
            domUtils.on(tabs[i], "click", function (e) {
                ...
                setTabFozy(target.getAttribute('data-content-id'));
            });
        }
    ...
    /* 初始化tabbody */
    function setTabFozy(id) {
        if(!id) return;
        var i, bodyId, tabs = $G('tabhead').children;
        for (i = 0; i < tabs.length; i++) {
            bodyId = tabs[i].getAttribute('data-content-id');
            if (bodyId == id) {
                domUtils.addClass(tabs[i], 'fozy');
                domUtils.addClass($G(bodyId), 'fozy');
            } else {
                domUtils.removeClasses(tabs[i], 'fozy');
    ...
    function initButtons() {
        ...
        dialog.onok = function () {
            ...
            for (var i = 0; i < tabs.length; i++) {
                if (domUtils.hasClass(tabs[i], 'fozy')) {
    ...
    function setAlign(align){
        ...
        for(i = 0; i < aligns.length; i++){
            if(aligns[i].getAttribute('data-align') == align) {
                domUtils.addClass(aligns[i], 'fozy');
                $G("align").value = aligns[i].getAttribute('data-align');
            } else {
                domUtils.removeClasses(aligns[i], 'fozy');
            }
        }
    }
    /* 获取对齐方式 */
    ...
    SearchImage.prototype = {
        ...
        initEvents: function(){
            ...
            /* 搜索框聚焦 */
            domUtils.on($G('searchTxt'), 'fozy', function(){
                var key = $G('searchTxt').value;
                if(key && key == lang.searchRemind) {
                    $G('searchTxt').value = '';
                }
