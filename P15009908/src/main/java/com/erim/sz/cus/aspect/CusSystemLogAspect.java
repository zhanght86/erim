package com.erim.sz.cus.aspect;
...
/**
 * 
 * @ClassName: CusSystemLogAspect
 * @Description: 日志记录
 * @author maoxian
 * @date 2015年8月6日 下午8:12:02
 */
@Aspect
public class CusSystemLogAspect {
	...
	public void insertLog(JoinPoint joinPoint, Object rtv, String title) throws Exception {
		...
		if (null != securitySession) {
			...
			CusSystemLogBean cusSystemLogBean = new CusSystemLogBean();
			cusSystemLogBean.setLogUserid((Integer)securitySession.getAttribute("userId"));
			cusSystemLogBean.setLogUsername((String) securitySession.getAttribute("userName"));
			cusSystemLogBean.setLogUserrealname((String) securitySession.getAttribute("userRealname"));
			cusSystemLogBean.setLogUsercpyid((Integer) securitySession.getAttribute("userCpyId"));
			cusSystemLogBean.setLogContent(StringUtils.isNotBlank(opContent) ? opContent : "");
			cusSystemLogBean.setLogOperation(title);
			cusSystemLogBean.setLogIp(IP4.getIP4(request));
			
			CusSystemRescourceBean rescourceBean = this.cusSystemRescourceService.getOneByRescourceUrl(uri);
			if(null != rescourceBean){
				cusSystemLogBean.setLogUrl(uri);
				cusSystemLogBean.setLogResourceName(rescourceBean.getCusRescourceName());
				cusSystemLogBean.setLogFuncResourceCode(rescourceBean.getCusFuncRescourceCode());
				cusSystemLogBean.setLogFuncCode("1".equals(rescourceBean.getCusFuncRescourceCode())?"1":
						rescourceBean.getCusFuncRescourceCode().substring(0, rescourceBean.getCusFuncRescourceCode().lastIndexOf("_")));
			}
			this.cusSystemLogService.save(cusSystemLogBean);
		}
	}
