...
import com.erim.sz.price.service.PriceCpyService;
 ...
 * @Description: 佣金管理
 * @author maoxian
 * @date 2015年11月3日 上午11:35:14
 */
@Controller
public class PriceCpyController {

	@Autowired
	private PriceCpyService      priceCpyService;
	@Autowired
	...
	/**
	 * @Title: show @Description: 佣金管理查看页面 @param @param model @param @return
	 * 设定文件 @return String 返回类型 @throws
	 */
	@RequestMapping(value = "/price/cpy/show")
	public String show(ModelMap model) {
		this.priceCpyService.showLine(model);
		return "/price/cpy/show";
	}

	/**
	 * @Title: payindex @Description: 主页 @param @param model @param @return
	 * 设定文件 @return String 返回类型 @throws
	 */
	@RequestMapping(value = "/price/cpy/index")
	public String payindex(ModelMap model) {
		return "/alipay/index";
	}
	...
	public String payali(HttpServletRequest request,ModelMap model) throws UnsupportedEncodingException {
		...
		// 必填，不能修改
		// 服务器异步通知页面路径
		String notify_url = "http://www.jialvlianhe.com/notify_url.jsp";
		// 需http://格式的完整路径，不能加?id=123这类自定义参数

		// 页面跳转同步通知页面路径
		String return_url = "http://www.jialvlianhe.com/return_url.jsp";
	...
	public String payByLoginname(HttpServletRequest request,ModelMap model){
		...
		if(StringUtils.isNotBlank(loginname)){
			...
			ZxSystemUserBean user = this.zxSystemUserService.selectUserByLoginname(loginname);
			if(null != user){
