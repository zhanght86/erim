...
(function(){
	...
	function build() {
		...
		svg.Element.TextElementBase = function(node) {
			...
			this.renderChildren = function(ctx) {
				var zytomFont = this.parent.style('font-family').Definition.getDefinition();
				if (zytomFont != null) {
					var fontSize = this.parent.style('font-size').numValueOrDefault(svg.Font.Parse(svg.ctx.font).fontSize);
					var fontStyle = this.parent.style('font-style').valueOrDefault(svg.Font.Parse(svg.ctx.font).fontStyle);
					var text = this.getText();
					if (zytomFont.isRTL) text = text.split("").reverse().join("");
					
					var dx = svg.ToNumberArray(this.parent.attribute('dx').value);
					for (var i=0; i<text.length; i++) {
						var glyph = this.getGlyph(zytomFont, text, i);
						var scale = fontSize / zytomFont.fontFace.unitsPerEm;
						ctx.translate(this.x, this.y);
						ctx.scale(scale, -scale);
						var lw = ctx.lineWidth;
						ctx.lineWidth = ctx.lineWidth * zytomFont.fontFace.unitsPerEm / fontSize;
						...
						this.x += fontSize * (glyph.horizAdvX || zytomFont.horizAdvX) / zytomFont.fontFace.unitsPerEm;
			...
			this.measureText = function(ctx) {
				...
				if (zytomFont != null) {
					...
					for (var i=0; i<text.length; i++) {
						var glyph = this.getGlyph(zytomFont, text, i);
						measure += (glyph.horizAdvX || zytomFont.horizAdvX) * fontSize / zytomFont.fontFace.unitsPerEm;
						if (typeof(dx[i]) != 'undefined' && !isNaN(dx[i])) {
							measure += dx[i];
						}
					}
