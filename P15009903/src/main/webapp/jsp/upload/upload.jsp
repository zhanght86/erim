	...
	if (isMultipart) {
		...
		while (iter.hasNext()) {
			...
			if (item.isFormField()) { // Process a regular form field
				...
				if("cpyid".equals(name)) cpyid = value;
				...
				if (fileName == null || fileName.length() == 0) {
					...
					String realpath = application.getRealPath("res/cpy/"+cpyid+"/upload/"+nowDate);
					...
					fout.write(data);
					fout.close();
					out.println("{\"filename\":\""+fileName+"\",\"url\":\"/cpy/"+cpyid+"/upload/"+nowDate+"/"+getShortFileName(fileName)+"\"}");
				}
			}
		}
	} else {
