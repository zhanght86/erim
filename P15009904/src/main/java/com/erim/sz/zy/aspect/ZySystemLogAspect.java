...
import com.erim.sz.zy.service.ZySystemRescourceService;
import com.erim.utils.ip.IP4;

/**
 * 
 * @ClassName: ZySystemLogAspect
 * @Description: 日志记录
 * @author maoxian
 * @date 2015年8月6日 下午8:12:02
 */
@Aspect
public class ZySystemLogAspect {

	@Autowired
	private ZySystemLogService       zySystemLogService;
	@Autowired
	private ZySystemRescourceService zySystemRescourceService;
	...
	public void insertLog(JoinPoint joinPoint, Object rtv, String title) throws Exception {
		...
		if (null != securitySession) {
			...
			ZySystemLogBean zySystemLogBean = new ZySystemLogBean();
			zySystemLogBean.setLogUserid((Integer)securitySession.getAttribute("userId"));
			zySystemLogBean.setLogUsername((String) securitySession.getAttribute("userName"));
			zySystemLogBean.setLogUserrealname((String) securitySession.getAttribute("userRealname"));
			zySystemLogBean.setLogUsercpyid((Integer) securitySession.getAttribute("userCpyId"));
			zySystemLogBean.setLogContent(StringUtils.isNotBlank(opContent) ? opContent : "");
			zySystemLogBean.setLogOperation(title);
			zySystemLogBean.setLogIp(IP4.getIP4(request));
			
			ZySystemRescourceBean rescourceBean = this.zySystemRescourceService.getOneByRescourceUrl(uri);
			if(null != rescourceBean){
				zySystemLogBean.setLogUrl(uri);
				zySystemLogBean.setLogResourceName(rescourceBean.getZyRescourceName());
				zySystemLogBean.setLogFuncResourceCode(rescourceBean.getZyFuncRescourceCode());
				zySystemLogBean.setLogFuncCode("1".equals(rescourceBean.getZyFuncRescourceCode())?"1":
						rescourceBean.getZyFuncRescourceCode().substring(0, rescourceBean.getZyFuncRescourceCode().lastIndexOf("_")));
			}
			this.zySystemLogService.save(zySystemLogBean);
		}
	}
