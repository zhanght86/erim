       ...
       // 将appRoot放入modelAndView
        ...
        if (SecurityUtils.getSubject().isAuthenticated()) {
            ...
            if (SecurityUtils.getSubject().getSession().getAttribute("userName") == null) {
                ...
                // 查询客户其他信息信息
            	ZxSystemUserBean bean = userDao.selectUserByLoginname(SecurityUtils.getSubject().getPrincipal().toString());
	            ...
	            //获取角色名称
	            SecurityUtils.getSubject().getSession().setAttribute("userRoleName", bean.getZxRoleId());
            	...
            	if(!uri.contains("/index;")){
            		//是否包含权限
                	boolean isHaveAuthorityByRoleUrl = this.zxSystemRescourceService.isHavaAuthorityByRoleUrl(uri);
