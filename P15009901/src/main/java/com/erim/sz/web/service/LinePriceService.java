...
import com.erim.sz.web.dao.LinePriceDao;
...
public class LinePriceService {
	 ...
	 * @param bean
	 */
	public void getLinePriceList(ModelMap map, LinePriceBean bean) {
		...
		// 执行查询
		List<LinePriceBean> modelList = linePriceDao.getLinePriceList(bean, monthCount);
		...
		map.addAttribute("tdlId", bean.getTdlId());
	}
	 ...
	 * @param bean
	 */
	public void forward(ModelMap map, LinePriceBean bean) {
	 ...
	 * @描述: 时间向后选择
	 * @作者: 宁晓强
	 * @创建时间: 2015年12月18日 下午4:30:17
	 * @param map
	 * @param bean
	 */
	public void backwards(ModelMap map, LinePriceBean bean) {
	 ...
	 * @return
	 */
	public List<LinePriceBean> createCalendar(List<LinePriceBean> modelList, String[] monthCount, String year, String month) {
		...
		if (modelList.size() != 0) {
			...
			for (int i = 0; i < modelList.size(); i++) {
				...
				// 房量信息日期
				String hnpDate = model.getLpeDate();
				// 下标
				...
				if (monthCount[j].equals(hnpDate)) {
					...
					//相差天数和提前预定天数做比较
					if(model.getLpeRestrict() != null){
						if(differenceDay - model.getLpeRestrict() >= 0){
							//可以预定
