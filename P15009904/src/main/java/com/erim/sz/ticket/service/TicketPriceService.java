...
public class TicketPriceService {
	...
	public Integer updatePrice(TicketPriceBean bean) {
		Integer result = CommonUtil.ERROR;
		String tplSign = bean.getTplSign();
		// 当数量 等于null 时，则设置为不限量
		if (tplSign == null || "".equals(tplSign) || "∞".equals(tplSign)) {
			bean.setTplIsSign(ErimConstants.YESORNO_NO);
			bean.setTplSign(null);
		} else {
			bean.setTplIsSign(ErimConstants.YESORNO_YES);
		}
