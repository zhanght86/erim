...
public class PubNewsDao extends BaseDao {
	...
	public List<PubNewsBean> selectPageNews(BaseBean bean, ModelMap map) {
		List<PubNewsBean> list = new ArrayList<PubNewsBean>();
		list = getSqlSession().selectList("pubnews.selectPageNews", bean, 
	...
	public Integer updateNews(PubNewsBean bean) {
		Integer result = CommonUtil.ERROR;
		result = getSqlSession().update("pubnews.update", bean);
		return result;
	}
	 ...
	 * @描述: 是否审核
	 * @作者: （李庆）
	 * @创建时间: 2015年12月18日 下午4:32:29
	 * @param bean
	 */
	public void newisdel(PubNewsBean bean) {
		 getSqlSession().update("pubnews.newisdel", bean);
	}
}
