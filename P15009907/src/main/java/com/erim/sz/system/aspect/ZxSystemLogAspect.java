...
import com.erim.sz.system.service.ZxSystemRescourceService;
import com.erim.utils.ip.IP4;

/**
 * 
 * @ClassName: ZxSystemLogAspect
 * @Description: 日志记录
 * @author maoxian
 * @date 2015年8月6日 下午8:12:02
 */
@Aspect
public class ZxSystemLogAspect {

	@Autowired
	private ZxSystemLogService       zxSystemLogService;
	@Autowired
	private ZxSystemRescourceService zxSystemRescourceService;
	...
	public void insertLog(JoinPoint joinPoint, Object rtv, String title) throws Exception {
		...
		if (null != securitySession) {
			...
			ZxSystemLogBean zxSystemLogBean = new ZxSystemLogBean();
			zxSystemLogBean.setLogUserid((Integer)securitySession.getAttribute("userId"));
			zxSystemLogBean.setLogUsername((String) securitySession.getAttribute("userName"));
			zxSystemLogBean.setLogUserrealname((String) securitySession.getAttribute("userRealname"));
			zxSystemLogBean.setLogUsercpyid((Integer) securitySession.getAttribute("userCpyId"));
			zxSystemLogBean.setLogContent(StringUtils.isNotBlank(opContent) ? opContent : "");
			zxSystemLogBean.setLogOperation(title);
			zxSystemLogBean.setLogIp(IP4.getIP4(request));
			
			ZxSystemRescourceBean rescourceBean = this.zxSystemRescourceService.getOneByRescourceUrl(uri);
			if(null != rescourceBean){
				zxSystemLogBean.setLogUrl(uri);
				zxSystemLogBean.setLogResourceName(rescourceBean.getZxRescourceName());
				zxSystemLogBean.setLogFuncResourceCode(rescourceBean.getZxFuncRescourceCode());
				zxSystemLogBean.setLogFuncCode("1".equals(rescourceBean.getZxFuncRescourceCode())?"1":
						rescourceBean.getZxFuncRescourceCode().substring(0, rescourceBean.getZxFuncRescourceCode().lastIndexOf("_")));
			}
			this.zxSystemLogService.save(zxSystemLogBean);
		}
	}
