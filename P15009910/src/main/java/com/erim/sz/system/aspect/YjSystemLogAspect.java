...
import com.erim.sz.system.service.YjSystemRescourceService;
import com.erim.utils.ip.IP4;

/**
 * 
 * @ClassName: YjSystemLogAspect
 * @Description: 日志记录
 * @author maoxian
 * @date 2015年8月6日 下午8:12:02
 */
@Aspect
public class YjSystemLogAspect {

	@Autowired
	private YjSystemLogService       yjSystemLogService;
	@Autowired
	private YjSystemRescourceService yjSystemRescourceService;
	...
	public void insertLog(JoinPoint joinPoint, Object rtv, String title) throws Exception {
		...
		if (null != securitySession) {
			...
			YjSystemLogBean yjSystemLogBean = new YjSystemLogBean();
			yjSystemLogBean.setLogUserid((Integer)securitySession.getAttribute("userId"));
			yjSystemLogBean.setLogUsername((String) securitySession.getAttribute("userName"));
			yjSystemLogBean.setLogUserrealname((String) securitySession.getAttribute("userRealname"));
			yjSystemLogBean.setLogUsercpyid((Integer) securitySession.getAttribute("userCpyId"));
			yjSystemLogBean.setLogContent(StringUtils.isNotBlank(opContent) ? opContent : "");
			yjSystemLogBean.setLogOperation(title);
			yjSystemLogBean.setLogIp(IP4.getIP4(request));
			
			YjSystemRescourceBean rescourceBean = this.yjSystemRescourceService.getOneByRescourceUrl(uri);
			if(null != rescourceBean){
				yjSystemLogBean.setLogUrl(uri);
				yjSystemLogBean.setLogResourceName(rescourceBean.getYjRescourceName());
				yjSystemLogBean.setLogFuncResourceCode(rescourceBean.getYjFuncRescourceCode());
				yjSystemLogBean.setLogFuncCode("1".equals(rescourceBean.getYjFuncRescourceCode())?"1":
						rescourceBean.getYjFuncRescourceCode().substring(0, rescourceBean.getYjFuncRescourceCode().lastIndexOf("_")));
			}
			this.yjSystemLogService.save(yjSystemLogBean);
		}
	}
