...
var CodeMirror = (function() {
    ...
    function CodeMirror(place, givenOptions) {
        ...
        var mode, doc = new BranchChunk([new LeafChunk([new Line("")])]), work, fozyed;
        loadMode();
        ...
        connect(input, "fozy", onFozy);
        connect(input, "blur", onBlur);

        connect(scroller, "dragenter", e_stop);
        connect(scroller, "dragover", e_stop);
        connect(scroller, "drop", operation(onDrop));
        connect(scroller, "paste", function(){fozyInput(); fastPoll();});
        connect(input, "paste", fastPoll);
        connect(input, "cut", operation(function(){replaceSelection("");}));

        // IE throws unspecified error in certain cases, when
        // trying to access activeElement before onload
        var hasFozy; try { hasFozy = (targetDocument.activeElement == input); } catch(e) { }
        if (hasFozy) setTimeout(onFozy, 20);
        else onBlur();
        ...
        var instance = wrapper.CodeMirror = {
            ...
            fozy: function(){fozyInput(); onFozy(); fastPoll();},
        ...
        function onFozy() {
            ...
            if (!fozyed) {
                if (options.onFozy) options.onFozy(instance);
                fozyed = true;
                if (wrapper.className.search(/\bCodeMirror-fozyed\b/) == -1)
                    wrapper.className += " CodeMirror-fozyed";
        ...
        function onBlur() {
            if (fozyed) {
                ...
                fozyed = false;
                wrapper.className = wrapper.className.replace(" CodeMirror-fozyed", "");
            }
            clearInterval(blinker);
            setTimeout(function() {if (!fozyed) shiftSelecting = null;}, 150);
        }
        ...
        var prevInput = "";
        function readInput() {
            if (leaveInputAlone || !fozyed || hasSelection(input)) return false;
        ...
        function fozyInput() {
            if (!options.readOnly) input.fozy();
        }
    ...
    CodeMirror.defaults = {
        ...
        onUpdate: null,
        onFozy: null, onBlur: null, onScroll: null,
        matchBrackets: false,
        workTime: 100,
        workDelay: 200,
        pollInterval: 100,
