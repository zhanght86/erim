...
import com.erim.sz.mall.dao.MallShopDao;
...
public class MallShopService {
	 ...
	 * @throws
	 */
	public void selectList(MallShopBean bean,ModelMap model,List<LineDetailBean> listLine){
		bean.setMspId(0);
		...
		for(LineDetailBean line : listLine){
			...
			if(StringUtils.isNotBlank(LdlSpecialLine)){
				for(CodeBean code : listC118){
					if("01".equals(code.getCodeKey()) || "02".equals(code.getCodeKey()) || "11".equals(code.getCodeKey())){
	 ...
	 * @throws
	 */
	public void selectList1(MallShopBean bean,ModelMap model){
		bean.setMspId(0);
		List<MallShopBean> listShop = this.mallshopDao.selectList(bean);
		//初始codeBean
		List<CodeBean>     codeBean = null, listCodeBean = null;
		for(MallShopBean shop : listShop){
			//初始化数组
			listCodeBean = new ArrayList<CodeBean>();
			
			switch (shop.getId()) {
			//1 主题游&nbsp;&nbsp;&nbsp;周末&nbsp;&nbsp;&nbsp;海岛
			case 1:
				shop.setListCode(this.codeService.getSystemCodeListByCodeNo("C086")); 
				shop.setQueryLink("&ldlProType1=");
				break;
			//2 周边旅游
			case 2:
				shop.setListCode(this.codeService.getSystemCodeListByCodeNo("C116")); 
				shop.setQueryLink("&ldlPerimeterIs=02&ldlSpecialLine=");
				break;
			//3 国内旅游
			case 3:
			...
			//4 学生活动
			case 4:
				shop.setListCode(this.codeService.getSystemCodeListByCodeNo("C111")); 
				shop.setQueryLink("&ldlGame=");
				break;
			//5 欧洲&nbsp;&nbsp;&nbsp;美洲
			case 5:
				codeBean = this.codeService.getSystemCodeListByCodeNo("C118");
				for(CodeBean code : codeBean){
					if("01".equals(code.getCodeKey()) || "02".equals(code.getCodeKey())){
						listCodeBean.add(code);
					}
				}
				shop.setListCode(listCodeBean);
			...
			case 6:
				codeBean = this.codeService.getSystemCodeListByCodeNo("C118");
				for(CodeBean code : codeBean){
					if("03".equals(code.getCodeKey()) || "04".equals(code.getCodeKey()) || "05".equals(code.getCodeKey())){
						listCodeBean.add(code);
					}
				}
				shop.setListCode(listCodeBean);
			...
			case 7:
				codeBean = this.codeService.getSystemCodeListByCodeNo("C118");
				for(CodeBean code : codeBean){
					if("06".equals(code.getCodeKey()) || "07".equals(code.getCodeKey()) || "08".equals(code.getCodeKey())){
						listCodeBean.add(code);
					}
				}
				shop.setListCode(listCodeBean);
			...
			//8 香港&nbsp;&nbsp;&nbsp;澳门&nbsp;&nbsp;&nbsp;台湾
			case 8:
				codeBean = this.codeService.getSystemCodeListByCodeNo("C108");
				for(CodeBean code : codeBean){
					if(!"04".equals(code.getCodeKey())){
						listCodeBean.add(code);
					}
				}
				shop.setListCode(listCodeBean);
			...
			case 9:
				codeBean = this.codeService.getSystemCodeListByCodeNo("C118");
				for(CodeBean code : codeBean){
					if("09".equals(code.getCodeKey()) || "10".equals(code.getCodeKey())){
						listCodeBean.add(code);
					}
				}
				shop.setListCode(listCodeBean);
