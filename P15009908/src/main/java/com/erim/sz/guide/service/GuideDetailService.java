...
public class GuideDetailService {
	...
	public void showGuideTown(ModelMap model, GuideDetailBean bean){
		...
		if("1".equals(CommonUtil.getCooperation())){
			Integer cid = cusSystemCooperationService.getCpyIdByNtype(ErimConstants.TOWN_GUIDE);
			//产品id
...
//			}
	...
	public Integer insert(ModelMap model, GuideDetailBean bean) {
		...
		if (!service.contains("03")) {
			...
			// 当包含国际港澳台领队，但是不包含国际
			if (!bean.getGdlDestination().contains("04")) {
				bean.setGdlDestinationCountry(null);
			}
		}
		// 不包含国际港澳台地陪
		if (!service.contains("04")) {
			...
			// 当包含国际港澳台地陪，但不包含国际
			if (!bean.getGdlGaidLokal().contains("04")) {
				bean.setGdlGaidCountry(null);
			}
		}
		
		// 获取语种是否包含其他
		String gdlLanguage = bean.getGdlLanguage();
		if (!gdlLanguage.contains("21")) {
			bean.setGdlRest("");
		}
	...
	public Integer update(ModelMap model, GuideDetailBean bean) {
		...
		try {
			...
			if (!service.contains("03")) {
				...
				// 当包含国际港澳台领队，但是不包含国际 、 国际编码04
				if (bean.getGdlDestination() == null || !bean.getGdlDestination().contains("04")) {
					bean.setGdlDestinationCountry(null);
				}
			}
			// 不包含国际港澳台地陪
			if (!service.contains("04")) {
				...
				// 当包含国际港澳台地陪，但不包含国际 、 国际编码04
				if (bean.getGdlGaidLokal() == null || !bean.getGdlGaidLokal().contains("04")) {
					bean.setGdlGaidCountry(null);
				}
			}
			...
			return guideDao.updateGuide(bean);
		} catch(Exception e) {
			return CommonUtil.error(getClass(), e);
		}
	}
