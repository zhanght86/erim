...
public class HotelDetailService {
	...
	public void selectPageHotelList(ModelMap map, HotelDetailBean bean) {
		...
		if("1".equals(CommonUtil.getCooperation())){
			Integer cid = cusSystemCooperationService.getCpyIdByNtype(ErimConstants.TOWN_HOTEL);
			//产品id
	 ...
	 * @描述: 打开预定方式页面
	 * @作者: 宁晓强
	 * @创建时间: 2015年12月18日 上午10:25:14
	 * @param map
	 * @param id
	 */
	public void showScheduledPage(ModelMap map, Integer id) {
		HotelDetailBean bean = new HotelDetailBean(); 
		if (id != null) {
			// 根据ID获取一条酒店基础信息
			bean = hotelDetailDao.getHotelDetailById(id);
	 ...
	 * @描述: 修改预定方式
	 * @作者: 宁晓强
	 * @创建时间: 2015年12月18日 上午10:28:48
	 * @param bean
	 * @return
	 */
	public Integer updateScheduled(HotelDetailBean bean) {
		Integer result = hotelDetailDao.updateScheduled(bean);
		return result;
	}
