...
public class ManagementDetailService {
	...
	public void showManagement(ModelMap model,ManagementDetailBean bean) {
		...
		if("1".equals(CommonUtil.getCooperation())){
			Integer cid = cusSystemCooperationService.getCpyIdByNtype(ErimConstants.TOWN_MANAGEMENT);
			//产品id
	...
	public void setCode(ModelMap model) {
		...
		//签证类型
		model.addAttribute("models", codeService.getSystemCodeByCodeNo(DictionaryUtil.NTYPE)); 
		//签证人
		model.addAttribute("attestor", codeService.getSystemCodeByCodeNo(DictionaryUtil.ATTESTOR));
		//入境次数
		model.addAttribute("entry", codeService.getSystemCodeByCodeNo(DictionaryUtil.ENTRYNUM));
		//面签需要
		model.addAttribute("face", codeService.getSystemCodeByCodeNo(DictionaryUtil.INTERVIEW));
		//预定方式
		model.addAttribute("scheduled", codeService.getSystemCodeByCodeNo(DictionaryUtil.SCHEDULE));
		//国家
		model.addAttribute("guojia", codeService.getSystemCodeListByCodeNo(DictionaryUtil.COUNTRY));
		//国家字符串类型 自动检索时用
        model.addAttribute("guojiaforquery", getGuojiaForQuery(codeService.getSystemCodeByCodeNo(DictionaryUtil.COUNTRY)));
		//送签地
		model.addAttribute("send", codeService.getSystemCodeByCodeNo(DictionaryUtil.SEND));
	}
