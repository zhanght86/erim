...
$(document).ready(function(){
	...
	$("#tpl_date_d li").on("click","a",function(){
		...
		if($(this).html()=="停售"){
			...
			if (tplIsSign == "01") {
				// 置灰
				$("#update_tplSign").attr('disabled','true');
				$("#update_tplSign").val(null);
				// 选中
				$("#update_tplIsSign").attr("checked","checked");
			} else {
				// 填值
				$("#update_tplSign").attr('disabled',false);
				$("#update_tplSign").val(tplSign);
				$("#update_tplIsSign").removeAttr("checked");
			}
